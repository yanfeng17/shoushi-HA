...
**v2.0.0 重大更新**：简化为纯手势识别系统，从 4 个手势扩展到 10 个静态手势，删除了表情识别和动态手势功能，代码精简 55%，性能提升 30%。
...
### 手势识别
- **10 种静态手势**：从基础手势（张开/握拳）到高级手势（点赞/剪刀手）
- **高精度识别**：使用 Full 模型，基于 21 个手部关键点的几何分析，精度提升 20-30%
- **可配置模型**：支持 Lite/Full/Heavy 三种模型，平衡性能和精度
- **防抖动机制**：支持连续检测次数和冷却时间配置，防止误触发
...
- **性能优化**：CPU 占用降低 30%，内存占用降低 40%

## 🖐️ 可识别的手势（v2.0.0 扩展到 10 种）

| 手势 | 描述 | 启用状态 | 用途示例 |
|------|------|---------|---------|
| 🖐️ **张开手掌** | 五指全部伸直 | ✅ 默认启用 | 开灯、播放 |
| ✊ **握拳** | 五指全部收起 | ✅ 默认启用 | 关灯、暂停 |
| ☝️ **食指向上** | 只有食指伸直 | ✅ 默认启用 | 音量增加、下一首 |
| 👌 **OK手势** | 拇指和食指圈起来 | ✅ 默认启用 | 确认操作 |
| 👍 **点赞** | 拇指向上，其他手指收起 | ✅ 默认启用 | 赞同、增加 |
| 👎 **点踩** | 拇指向下，其他手指收起 | ✅ 默认启用 | 反对、减少 |
| ✌️ **剪刀手** | 食指和中指伸出 | ✅ 默认启用 | 选项 2、胜利 |
| 🤟 **三指** | 拇指+食指+中指伸出 | ⚠️ 可选启用 | 选项 3 |
| 🤚 **四指** | 食指+中指+无名指+小指 | ⚠️ 可选启用 | 选项 4 |
| 🤏 **捏合** | 拇指和食指尖端靠近 | ⚠️ 可选启用 | 精确选择、缩小 |

### 手势切换最佳实践

- ✅ **推荐**：移开手 → 停顿 0.5 秒 → 快速做新手势
- 🚫 **避免**：慢慢从一个手势变到另一个手势
- 💡 **技巧**：手势要标准，比如 PEACE 手势的无名指和小指要紧贴手掌
...
### MediaPipe 模型（v2.0.0 新增）

| 参数 | 默认值 | 选项 | 说明 |
|------|--------|------|------|
| `model_complexity` | 1 | 0/1/2 | 0=Lite（快），1=Full（推荐⭐），2=Heavy（高精度）|

**模型对比**：
- **Lite (0)**：最快，精度较低，适合低端设备
- **Full (1)**：平衡，精度高（比 Lite 提升 20-30%），**推荐**
- **Heavy (2)**：最慢，精度极高（比 Full 再提升 10-15%）

### 手势开关（v2.0.0 全新）

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `enable_open_palm` | true | 启用"张开手掌" 🖐️ |
| `enable_closed_fist` | true | 启用"握拳" ✊ |
| `enable_pointing_up` | true | 启用"食指向上" ☝️ |
| `enable_ok_sign` | true | 启用"OK手势" 👌 |
| `enable_thumbs_up` | true | 启用"点赞" 👍 |
| `enable_thumbs_down` | true | 启用"点踩" 👎 |
| `enable_peace` | true | 启用"剪刀手" ✌️ |
| `enable_three_fingers` | false | 启用"三指" 🤟 |
| `enable_four_fingers` | false | 启用"四指" 🤚 |
| `enable_pinch` | false | 启用"捏合" 🤏 |
...
- ❌ 动态手势相关（8 个参数）
- ❌ 调试可视化相关（3 个参数）
- ❌ Blendshapes 相关（2 个参数）

## 🏗️ 系统架构

```
main.py                              # 主应用程序循环（v2.0.0 简化版）
├── VideoStreamProcessor             # RTSP 流处理与自动重连
├── GestureEngine                    # MediaPipe Hands 封装
│   ├── 模型配置                     # Lite/Full/Heavy 可选
│   ├── 10 种静态手势识别            # 基于几何关键点分析
│   └── 手势优先级检测               # 防止误识别
...
- **删除**：ExpressionEngine（表情引擎）
- **删除**：MotionDetector（动态手势检测器）
- **删除**：DebugVisualizer（调试可视化）
- **代码量**：从 1543 行减少到 690 行（-55%）
...
- **拇指状态**：计算与手掌中心的距离，判断方向（向上/向下）
- **几何关系**：特定关键点间的距离（如拇指-食指距离判断 OK 手势和捏合手势）
- **优先级检测**：按优先级顺序检测手势，防止误识别（如 PEACE 优先于 OPEN_PALM）
...
**Full 模型**（推荐配置）：
- FPS：15-20 @ 320×240
- CPU：30-40%（比 v1.0.8 降低 30%）
- 内存：~300-400 MB（比 v1.0.8 降低 40%）
- 响应时间：0.10-0.15 秒
...
**提升准确性**：
```yaml
model_complexity: 2          # 使用 Heavy 模型
frame_width: 640
frame_height: 480
skip_frames: 1              # 处理所有帧
